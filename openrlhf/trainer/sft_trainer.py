            ...
            for prompt_id_lens, inputs, attention_masks, infos in self.train_dataloader:
                ...
                if not self.pretrain_mode:
                    if self.packing_samples:
                        ...
                        if infos["response_ranges"]:
                            dump_labels = torch.full(labels.size(), self.loss_fn.IGNORE_INDEX).to(labels.device)
                            for i in range(len(infos["response_ranges"])): # bs
                                for response_range in infos["response_ranges"][i]: 
                                    dump_labels[i][response_range[0]: response_range[1]] = labels[i][response_range[0]: response_range[1]]
                            labels = dump_labels
                        else:
                ...
                if print_example:
                    ...
                    if not self.packing_samples:
                        self.strategy.print("==========> An Example of Input")
                        self.strategy.print(self.tokenizer.decode(inputs[0].squeeze()))
                    print_example = False
