...
class RewardModelTrainer(ABC):
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        # initial_kl_loss_coef = 0
        # max_kl_loss_coef = args.kl_loss_coef
        # annealing_rate = (max_kl_loss_coef - initial_kl_loss_coef) / num_update_steps_per_epoch
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    ...
                    chosen_reward, reject_reward, aux_loss, kl_loss, output = self.concatenated_forward(
                    ...
                    chosen_reward, reject_reward, aux_loss, kl_loss, output = self.packed_samples_forward(
                ...
                if not self.args.use_inform:
                    kl_loss = torch.tensor(0)
                else:
                    assert kl_loss!= [], f"kl_loss is {kl_loss}"

                # kl_weight = initial_kl_loss_coef + annealing_rate * step
                # loss = preference_loss + aux_loss * self.args.aux_loss_coef + kl_loss * kl_weight
                loss = preference_loss + aux_loss * self.args.aux_loss_coef + kl_loss * self.args.kl_loss_coef
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask):
        ...
        kl_loss = output.kl_loss if "kl_loss" in output else []
        return chosen_rewards, rejected_rewards, aux_loss, kl_loss, output
