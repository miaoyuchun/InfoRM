...
class RewardDataset(Dataset):
    ...
    def process_data(self, data):
        ...
        # Filter the sample whose total length is greater than max_length
        _, _, chosen_token_len = self.concate_eos_and_tokenize(prompt, chosen)
        _, _, reject_token_len = self.concate_eos_and_tokenize(prompt, reject)

        if reject_token_len >= self.max_length or chosen_token_len >= self.max_length:
            prompt = None
    ...
    def __getitem__(self, idx):
        ...
        chosen, chosen_token, chosen_token_len = self.concate_eos_and_tokenize(prompt, chosen)
        reject, reject_token, reject_token_len = self.concate_eos_and_tokenize(prompt, reject)

        # assurt no over long samples
        assert chosen_token["input_ids"][0][-1] == self.tokenizer.eos_token_id and reject_token["input_ids"][0][-1] == self.tokenizer.eos_token_id, "EOS_token truncation!!"
        assert chosen_token["attention_mask"][0][-1] == True and reject_token["attention_mask"][0][-1] == True, "EOS_token truncation!!"
    ...
    def concate_eos_and_tokenize(self, prompt, chosen_or_rejected):
        text = (prompt + chosen_or_rejected).rstrip("\n")
        ...
        return text, input_token, text_ids_len