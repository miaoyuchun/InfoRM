...
prefix=step2_llama3_8b_sharegptsft_hh105_lr${lr}_bs${bs}_gbs${gbs}_epoch${epoch}_samples${samples}_chat_template_packing
bash ./example_rm/script/train_rm_llama3_chat_template_wprompt_packing.sh $zero_stage $lr $bs $gbs $pretrain $dataset $epoch $samples $output_dir $prefix